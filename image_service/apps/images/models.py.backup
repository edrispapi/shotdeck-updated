...
class Image(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            if self.movie and self.image_url:
                ...
                base_slug = slugify(self.title) if self.title else str(self.uuid)[:12]
    ...
    def analyze_colors(self, force=False):
        ...
        if not force and self.primary_color_hex:
            logger.info(f"Image {self.uuid} already has color analysis")
            return True
        ...
        try:
            ...
            logger.info(f"Successfully analyzed colors for image {self.uuid}")
            return True

        except Exception as e:
            logger.error(f"Error analyzing colors for {self.uuid}: {e}")
            return False
