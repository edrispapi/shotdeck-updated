...
class Command(BaseCommand):
    help = "Complete Shotdeck data import with ALL metadata fields"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("Complete Shotdeck Data Import with ALL Metadata")
    ...
    def _get_available_files(self):
        ...
        try:
            files = [f for f in os.listdir(self.image_dir) if f.lower().endswith(('.jpg', '.jpeg'))]
            return set(files)
        except Exception as e:
    ...
    def _process_json_file(self, json_file: str):
        """Process a single JSON file with complete metadata extraction"""
        ...
        # Get the actual image filename from JSON
        actual_image_file = data.get('data', {}).get('image_file', '')
        if not actual_image_file:
            raise Exception(f"No image_file field in JSON: {imageid}")
        
        # Find matching image file using the actual filename from JSON
        image_file = self._find_matching_file_fast(actual_image_file.replace('.jpg', ''))
        if not image_file:
            if self.skip_missing_files:
                self.stats['missing_files'] += 1
                return
            else:
                raise Exception(f"Image file not found: {actual_image_file}")
    ...
    def _find_matching_file_fast(self, imageid: str) -> Optional[str]:
        ...
        for file in self.available_files:
            if file.lower().startswith(imageid.lower()) and file.lower().endswith(('.jpg', '.jpeg')):
                return file
